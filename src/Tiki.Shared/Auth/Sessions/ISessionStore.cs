...
public interface ISessionStore
{
    ...
    /// <summary>
    /// Rewrites the permission set on every live session of a user, in place, preserving
    /// each session's remaining TTL. Called when a role changes, so the change takes effect
    /// on the user's next request rather than at their next login.
    /// </summary>
    Task<int> UpdatePermissionsAsync(Guid userId, IReadOnlyList<string> permissions, CancellationToken ct = default);
